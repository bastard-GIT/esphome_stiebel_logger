...
static const CanMember CanMembers[] =
    {
        ...
        {"NEUE_ID",       0x100, {0x21, 0x00}, {0x20, 0x00}, {0x00, 0x00}}, // Neuer Eintrag für 0x100
...
const ElsterIndex *processCanMessage(const std::vector<uint8_t> &msg, uint32_t can_id, std::string &signalValue)
{
    ...
    if ((msg[2] == 0xfa) || (msg[1] == 0xF8) || (msg[1] == 0xF9))
    {
        byte1 = msg[5];
        byte2 = msg[6];
...
void identifyCanMembers()
{
    ...
    for (int i = 0; i < 22; i++)
    {
        readSignal(&CanMembers[i], GetElsterIndex("PROGRAMMSCHALTER"));
