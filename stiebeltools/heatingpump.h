...
#old:	700	confirmation: E200
#old:	201 read: 3100  write: 3000
#################################################################
*/
...
//  Name              CanId     ReadId          WriteId         ConfirmationID
  { "ESPCLIENT"     , 0x680,    {0x00, 0x00},   {0x00, 0x00},   {0xD2, 0x00}}, //The ESP Home Client, thus no valid read/write IDs
...
const ElsterIndex *  processCanMessage(unsigned short can_id, std::string &signalValue, std::vector<unsigned char> msg)
{
    ...
    ESP_LOGI("processCanMessage()", "%03X:\t%s(0x%04X):\t%s\t(%s)", can_id, ei->Name, ElsterIndex_id, charValue, ElsterTypeStr[ei->Type]);
